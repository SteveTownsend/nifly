...
TEST_CASE("Load and save file (SF)", "[NifFile]") {
	...
	for (auto& s : shapes) {
		...
		for (auto meshPath : meshPaths) {
			...
			const std::filesystem::path meshInputPath(meshFileInput);
			...
			const std::filesystem::path meshOutputPath(meshFileOutput);
			...
			const std::filesystem::path meshExpectedPath(meshFileExpected);
